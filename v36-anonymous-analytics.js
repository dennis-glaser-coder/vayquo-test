...
const EVENT_SCHEMAS=Object.freeze({
  page_view:[],
  setup_category_select:['category'],
  setup_build:['category','budget_bucket','mode','goal_count','owned_count'],
  setup_share:['category','budget_bucket'],
  setup_shop_click:['category','target_host'],
  preset_open:['preset'],
...
const budgetBucket=()=>{
  const n=Number(document.getElementById('budget')?.value||0);
  if(!Number.isFinite(n)||n<=0)return '';
  if(n<500)return 'under_500';
  if(n<1000)return '500_999';
  if(n<1500)return '1000_1499';
  if(n<2500)return '1500_2499';
  if(n<4000)return '2500_3999';
  return '4000_plus';
};
...
function sanitize(eventName,properties={}){
  ...
  for(const key of allowed){
    let value=clean(properties?.[key],120);
    if(!value)continue;
    if(['category','budget_bucket','mode','goal_count','owned_count','preset'].includes(key)&&!/^[A-Za-z0-9._-]+$/.test(value))continue;
...
function send(eventName,properties={}){
  try{
    const safeProperties=sanitize(eventName,properties);
...
function presetName(){
  ...
  const m=p.match(/\/setups\/([a-z0-9-]+)\.html$/i);
  return m?safeToken(m[1],80):'';
}
...
document.addEventListener('click',event=>{
  ...
  if(target.matches('#build'))send('setup_build',{category:activeCategory(),budget_bucket:budgetBucket(),mode:currentMode(),goal_count:goalCount(),owned_count:ownedCount()});
  ...
  if(target instanceof HTMLAnchorElement){
    try{
      ...
      if(target.matches('.shop')&&u.origin!==location.origin){
        send('setup_shop_click',{category:activeCategory(),target_host:u.hostname.slice(0,120)});
      }else if(u.origin!==location.origin&&/^https?:$/.test(u.protocol)){
        send('outbound_click',{target_host:u.hostname.slice(0,120)});
      }
    }catch{}
  }
},true);
...
if(preset)send('preset_open',{preset});
