...
const EVENT_SCHEMAS=Object.freeze({
  ...
  revenue_intent:['category','route','budget_bucket'],revenue_flow_start:['category','route','budget_bucket'],revenue_result:['category','route','budget_bucket'],revenue_primary_click:['category','route','budget_bucket']
});
...
function makeBudgetBucket(value){const n=Number(value||0);if(!Number.isFinite(n)||n<=0)return'';if(n<1000)return'under_1000';if(n<2500)return'1000_2499';if(n<5000)return'2500_4999';if(n<10000)return'5000_9999';if(n<20000)return'10000_19999';if(n<40000)return'20000_39999';return'40000_plus'}
const budgetBucket=()=>makeBudgetBucket(document.getElementById('budget')?.value||document.getElementById('vqBudget')?.value||0);
...
window.VAYQUO_ANALYTICS={track:(name,props={})=>send(name,props),budgetBucket:makeBudgetBucket};
})();