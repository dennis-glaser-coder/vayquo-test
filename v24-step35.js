...
function platinumUsed(){return Math.min(200,+state?.benefits?.travel||0)+Math.min(150,+state?.benefits?.restaurant||0)+sixtUsedValue()+Math.min(50,+state?.benefits?.lodenH1||0)+Math.min(50,+state?.benefits?.lodenH2||0);}
...
function openManager(){
 ...
 <div class="v24s35-manager-note"><strong>Deine Angaben dienen zuerst deiner persönlichen Einordnung.</strong> Sie steuern, was VAYQUO überhaupt als deinen eigenen Vorteil anzeigen darf.</div>
 ...
 q('#v24s35-save')?.addEventListener('click',()=>{
  ...
  typeof save==='function'&&save();typeof closeModal==='function'&&closeModal();typeof render==='function'&&render();typeof toast==='function'&&toast('Setup aktualisiert');
 });
}

function setupChips(){
 const chips=[];
 if(hasAmexCard())chips.push(`<span class="v24s35-chip"><b>AX</b>${esc(cardName())}</span>`);else if(state.programs.mr)chips.push(`<span class="v24s35-chip"><b>MR</b>Membership Rewards</span>`);
...
function recommendation(){
 ...
 if(state.programs.mm&&statusKey()==='none')return {kind:'mm-status',eyebrow:'MEHR AUS DEINEM SETUP',title:'Dir fehlen aktuell Statusvorteile – nicht Meilen.',copy:'VAYQUO erkennt Miles & More, aber keinen Vielfliegerstatus. Deshalb werden Lounge- und Priority-Vorteile bewusst nicht als vorhanden angezeigt.',reason:'Das ist keine Kartenwerbung: zuerst sollte dein tatsächlicher Miles-&-More-Status geklärt werden.',cta:'Statusmöglichkeiten ansehen'};
 if(state.programs.mr&&!hasAmexCard())return {kind:'complete-amex',eyebrow:'SETUP VERVOLLSTÄNDIGEN',title:'Membership Rewards ist aktiv, aber die Karte fehlt.',copy:'Wenn du deine Amex-Karte ergänzt, kann VAYQUO Kartenvorteile korrekt zuordnen – ohne dir automatisch eine neue Karte zu empfehlen.',reason:'Mehr Daten statt mehr Werbung.',cta:'Karte ergänzen'};
 if(state.programs.pb&&!hasAmexCard())return {kind:'payback-amex',eyebrow:'PASSENDE ERGÄNZUNG · OPTIONAL',title:'Du könntest PAYBACK auch bei normalen Kartenzahlungen sammeln.',copy:'Eine PAYBACK-Karte kann dein bestehendes Programm ergänzen. VAYQUO zeigt sie hier nur, weil PAYBACK bereits zu deinem Setup gehört.',reason:'Warum du das siehst: PAYBACK ist aktiv, aber keine American-Express-Karte ist hinterlegt.',cta:'Für mich prüfen'};
 return null;
}
...
function openReco(kind){
 ...
 q('#v24s35-out')?.addEventListener('click',()=>openExternal(LINKS.paybackAmex));q('#v24s35-close')?.addEventListener('click',()=>typeof closeModal==='function'&&closeModal());
}

function bindCommon(root=document){
 qa('[data-v24s35-manage]',root).forEach(b=>b.onclick=openManager);
 qa('[data-v24s35-action]',root).forEach(b=>b.onclick=()=>{const a=b.dataset.v24s35Action;if(a==='payback'&&typeof go==='function')go('optimize','payback');if(a==='flight'&&typeof go==='function')go('optimize','flight');if(a==='mr'&&typeof go==='function')go('optimize','transfer');if(a==='mm-status')openExternal(MM_STATUS[statusKey()].url);});
...
function enhancePlatinum(screen){
 screen.dataset.v24s35='1';const head=q('.v19-head',screen);
 if(head){const button=q('#v23Switch',head);if(button)button.textContent='Setup ändern';head.insertAdjacentHTML('afterend',setupChips());}
 const summary=q('.v19-benefits',screen);if(summary)summary.insertAdjacentHTML('beforeend',`<div class="v24s35-sync">Offener Betrag laut deinem VAYQUO-Stand · nicht automatisch mit Amex oder SIXT synchronisiert.</div>`);
 const extras=[paybackBlock(),mmBlock()].filter(Boolean).join('');if(extras)screen.insertAdjacentHTML('beforeend',`<div class="v24s35-section v24s35-platinum-extra"><div class="v24s35-section-head"><h2>Weitere Programme</h2><span>gehören ebenfalls zu dir</span></div><div class="v24s35-owned">${extras}</div></div>`);
 bindCommon(screen);
}
function benefitsViewActive(){const nav=q('#bottom [data-view="card"]');if(nav?.classList.contains('active'))return true;const h=q('#app h1');return !!h&&h.textContent.trim()==='Vorteile';}
function enhanceBenefits(){if(!benefitsViewActive())return;const screen=q('#app .screen');if(!screen||screen.dataset.v24s35)return;if(state.card==='platinum')enhancePlatinum(screen);else renderNonPlatinum();}

document.addEventListener('click',ev=>{const hit=ev.target.closest?.('#v20ManageInline,#v20ManageSources,#v23Switch,#switchCard,[data-v24s35-manage]');if(!hit)return;ev.preventDefault();ev.stopImmediatePropagation();openManager();},true);
let scheduled=false;function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;enhanceBenefits();});}
