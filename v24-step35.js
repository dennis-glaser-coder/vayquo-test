...
const CARD_BENEFITS={
 gold:['Bis zu 370 € jährliche Guthaben','Reiseversicherungen','1 Zusatzkarte inklusive'],
 goldrose:['Bis zu 370 € jährliche Guthaben','Reiseversicherungen','1 Zusatzkarte inklusive'],
 green:['Membership Rewards inklusive','1 Zusatzkarte inklusive','Ab 9.000 € Umsatz: Folgejahr beitragsfrei'],
 blue:['Dauerhaft kein Jahresentgelt','Amex Offers','1 Zusatzkarte inklusive'],
 payback:['PAYBACK Punkte bei Kartenzahlungen','Dauerhaft kein Jahresentgelt','Zusatzkarten inklusive'],
 dmpayback:['PAYBACK Punkte bei Kartenzahlungen','Dauerhaft kein Jahresentgelt','Zusatzkarten inklusive'],
 bmw:['1 % Gutschrift beim Tanken & Laden','Fahrzeug-Schutzbrief','Membership Rewards inklusive'],
 bmwpremium:['Priority Pass inklusive','Umfangreiches Versicherungspaket','4 Zusatzkarten inklusive']
};
const MM_STATUS={
...
const LINKS={
 ...
 platinum:'https://www.americanexpress.com/de-de/kreditkarte/platinum-card/'
};
...
function benefitTags(items){return items?.length?`<div class="v24s35-benefit-tags">${items.map(x=>`<span>✓ ${esc(x)}</span>`).join('')}</div>`:'';}
...
function openManager(){
 ...
 openModal('Dein Setup',`<div class="v24s35-manager-intro">Programme, Karte und Status auswählen.</div>
 <div class="v20-modal-grid v24s35-manager-grid">
  ...
  <label class="v20-modal-item"><span class="mi">PB</span><span><strong>PAYBACK</strong><span>Punkte und Einlösen</span></span><input type="checkbox" data-v24s35-prog="pb" ${hasProgram('pb')?'checked':''}></label>
  <label class="v20-modal-item"><span class="mi">M&M</span><span><strong>Miles & More</strong><span>Meilen und Status</span></span><input type="checkbox" data-v24s35-prog="mm" ${mm?'checked':''}></label>
 </div>
 ...
 <div class="field" id="v24s35-mm-wrap" ${mm?'':'hidden'}><label>Welchen Miles-&-More-Status hast du?</label><select class="select" id="v24s35-mm">${mmOptions()}</select></div>
...
function mmBlock(){
 ...
 const benefits=info.benefits.length?benefitTags(info.benefits):`<div class="v24s35-status-empty"><strong>Kein Status hinterlegt.</strong><span>Status ergänzen, damit Lounge- und Reisevorteile korrekt erscheinen.</span></div>`;
...
function amexBlock(){
 ...
 if(!hasAmexCard()){
  return `<div class="v24s35-source-card" data-source="amex"><div class="v24s35-source-head"><span class="v24s35-monogram">MR</span><div><small>MEMBERSHIP REWARDS</small><strong>${integer(state?.balances?.mr)} Punkte</strong></div><div class="v24s35-source-value"><b>Karte offen</b><span>Setup</span></div></div><button type="button" class="v24s35-action" data-v24s35-manage>Karte ergänzen <span>→</span></button></div>`;
 }
 const card=String(state?.card||'none');
 const paybackCard=card==='payback'||card==='dmpayback';
 const side=paybackCard?'<b>aktiv</b><span>Karte</span>':`<b>${integer(state?.balances?.mr)}</b><span>MR Punkte</span>`;
 const action=paybackCard?'':`<button type="button" class="v24s35-action" data-v24s35-action="mr">Membership Rewards prüfen <span>→</span></button>`;
 return `<div class="v24s35-source-card" data-source="amex"><div class="v24s35-source-head"><span class="v24s35-monogram">AX</span><div><small>AMERICAN EXPRESS</small><strong>${esc(cardName())}</strong></div><div class="v24s35-source-value">${side}</div></div>${benefitTags(CARD_BENEFITS[card]||['Amex Offers'])}${action}</div>`;
}

function platinumHighlights(){
 const loungeAlready=(hasProgram('mm')&&statusKey()!=='none')||String(state?.card||'')==='bmwpremium';
 return loungeAlready
  ?['200 € Online-Reiseguthaben','150 € Restaurantguthaben','200 € SIXT ride Guthaben']
  :['200 € Online-Reiseguthaben','150 € Restaurantguthaben','Airport-Lounges weltweit'];
}

function recommendation(){
 if(ownsPlatinum())return null;
 if(hasProgram('mr')&&!hasAmexCard())return null;
 if(hasProgram('mr')&&hasAmexCard())return {kind:'platinum',eyebrow:'AMERICAN EXPRESS PLATINUM',title:'Mit Platinum zusätzlich',items:platinumHighlights(),cta:'Platinum ansehen'};
 if(hasProgram('mm')&&statusKey()!=='none')return {kind:'platinum',eyebrow:'AMERICAN EXPRESS PLATINUM',title:'Mit Platinum zusätzlich',items:platinumHighlights(),cta:'Platinum ansehen'};
 if(hasProgram('pb')&&!hasProgram('mr'))return {kind:'payback-amex',eyebrow:'PAYBACK AMERICAN EXPRESS',title:'Mit der PAYBACK Amex zusätzlich',items:['Bei Kartenzahlungen PAYBACK Punkte sammeln','Dauerhaft kein Jahresentgelt','Amex Offers nutzen'],cta:'PAYBACK Amex ansehen'};
 return null;
}

function recommendationHtml(){
 const r=recommendation();if(!r)return '';
 return `<div class="v24s35-section"><div class="v24s35-section-head"><h2>Mehr für dich</h2><span>optional</span></div><div class="v24s35-reco"><div class="v24s35-reco-eyebrow">${esc(r.eyebrow)}</div><h3>${esc(r.title)}</h3><div class="v24s35-reco-check">${r.items.map(x=>`<span>✓ ${esc(x)}</span>`).join('')}</div><button type="button" class="v24s35-reco-btn" data-v24s35-reco="${r.kind}">${esc(r.cta)} <span>→</span></button></div></div>`;
}

function openReco(kind){
 if(typeof openModal!=='function')return;
 if(kind==='payback-amex'){
  openModal('PAYBACK Amex',`<div class="v24s35-reco-modal"><div class="v24s35-reco-modal-kicker">MIT DER PAYBACK AMEX ZUSÄTZLICH</div><h3>Mehr Punkte aus normalen Kartenzahlungen.</h3><div class="v24s35-reco-check"><span>✓ PAYBACK Punkte bei Kartenzahlungen</span><span>✓ dauerhaft kein Jahresentgelt</span><span>✓ Amex Offers</span></div><div class="v24s35-reco-disclosure">Weitere Entgelte können gemäß AGB anfallen. Details und Bedingungen bei American Express.</div><button class="btn" id="v24s35-out">PAYBACK Amex ansehen ↗</button></div>`);
  q('#v24s35-out')?.addEventListener('click',()=>openExternal(LINKS.paybackAmex));
  return;
 }
 if(kind==='platinum'){
  const items=platinumHighlights();
  openModal('Amex Platinum',`<div class="v24s35-reco-modal"><div class="v24s35-reco-modal-kicker">MIT PLATINUM ZUSÄTZLICH</div><h3>Mehr Reisevorteile in deinem Setup.</h3><div class="v24s35-reco-check">${items.map(x=>`<span>✓ ${esc(x)}</span>`).join('')}</div><div class="v24s35-reco-disclosure">60 € monatliches Kartenentgelt. Details und Bedingungen bei American Express.</div><button class="btn" id="v24s35-out">Platinum ansehen ↗</button></div>`);
  q('#v24s35-out')?.addEventListener('click',()=>openExternal(LINKS.platinum));
 }
}
...
function renderNonPlatinum(){
 ...
 app.innerHTML=`<section class="screen v24s35-benefits" data-v24s35="1"><div class="v19-head v24s35-head"><div><div class="over">DEINE VORTEILE</div><h1>Vorteile</h1></div><button class="v19-card-change" data-v24s35-manage>Ändern</button></div>${setupChips()}<div class="v24s35-section"><div class="v24s35-section-head"><h2>Das hast du</h2></div><div class="v24s35-owned">${owned||'<div class="v24s35-empty">Noch keine Vorteile hinterlegt.</div>'}</div></div>${recommendationHtml()}</section>`;
 bindCommon(app);
}
...
function enhancePlatinum(screen){
 ...
 if(extras)screen.insertAdjacentHTML('beforeend',`<div class="v24s35-section v24s35-platinum-extra"><div class="v24s35-section-head"><h2>Weitere Programme</h2></div><div class="v24s35-owned">${extras}</div></div>`);
 bindCommon(screen);
}
