...
function mount(){
 ...
 root=document.createElement('div');root.id='v24-auth';
 root.innerHTML=`<section class="v24a-card"><div class="v24a-brand">VAYQUO</div><h1>Mehr aus deinen Punkten machen.</h1><p class="v24a-sub">Deine Punkte, Meilen und Vorteile an einem Ort.</p><div class="v24a-tabs"><button class="v24a-tab active" type="button" data-v24a-mode="login">Anmelden</button><button class="v24a-tab" type="button" data-v24a-mode="register">Registrieren</button></div><form id="v24a-form"><label class="v24a-field"><span>E-Mail</span><input id="v24a-email" type="email" autocomplete="email" inputmode="email" required></label><label class="v24a-field"><span>Passwort</span><input id="v24a-password" type="password" autocomplete="current-password" minlength="8" required></label><div class="v24a-msg" id="v24a-msg" role="status"></div><button class="v24a-primary" id="v24a-submit" type="submit">Anmelden</button><p class="v24a-note">Deine VAYQUO-Daten werden deinem Konto zugeordnet und nicht zwischen Nutzern geteilt.</p></form></section>`;
...
function hideGate(){
 q('#v24-auth')?.setAttribute('hidden','');document.documentElement.classList.remove('vq-auth-pending');
}
function showGate(){
 document.documentElement.classList.add('vq-auth-pending');mount().removeAttribute('hidden');setMode('login');
}
async function startAuthenticated(){
 await ensureSession();if(!session||!user){showGate();return;}
