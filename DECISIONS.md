...
## D-021 – Provider-Game-Verfügbarkeit ist Markt- und Produktdaten
Das Datenbankschema behandelt eine Spielverfügbarkeit nicht mehr als globale Eigenschaft eines Anbieters. `spielmatch_provider_games` trägt Markt, Produktvertikale und Evidenztyp; neue Datensätze beginnen `pending`. Verifizierte Datensätze brauchen Prüfdatum, HTTPS-Quelle und starke Evidenz. Öffentliche Reads des aktuellen MVP sind zusätzlich auf DE + `virtual_slots` + verifizierte Verfügbarkeit + aktiven GGL-verifizierten Anbieter begrenzt. Eine Live-Migration wird ohne Freigabe nicht ausgeführt.

## D-022 – Echte Matchkarten brauchen Mehranbieter-Überschneidung
Ein verifiziertes Spiel bei nur einem Anbieter reicht für Autocomplete und SEO-Evidenz, aber nicht für einen sinnvollen Anbieter-Vergleich. Für Kernspiele wird deshalb vor der Ablösung der Demo-Matchkarten eine Zielabdeckung von mindestens drei separat verifizierten DE-Anbietern verwendet. Am 05.09.2026 erreichen Book of Dead, Big Bass Bonanza und Big Bass Splash diese Schwelle: JackpotPiraten ergänzt Book of Dead, Big Bass Bonanza und Big Bass Splash über eigene Betreiber-Spielseiten; SlotMagie ergänzt Big Bass Splash. Der Seed steigt dadurch auf 14 verifizierte Beziehungen. Die Schwelle ist ein Produktqualitäts-Gate, keine rechtliche oder Affiliate-Freigabe und beeinflusst das Ranking nicht.

## D-023 – Kernspiel-Abdeckung gezielt statt nur Seed-Masse erhöhen
Zusätzliche Spiel→Anbieter-Beziehungen werden danach priorisiert, ob sie einen relevanten Suchintent über das Vergleichsgate bringen. Am 05.09.2026 wird Book of Ra mit aktuellen Betreiber-Spielseiten bei NOVOLINE, BingBong und JackpotPiraten ergänzt und erreicht zusammen mit SlotMagie vier verifizierte DE-Anbieter. Book of Ra Deluxe wird mit BingBong und JackpotPiraten ergänzt und erreicht zusammen mit SlotMagie drei. Der Seed umfasst damit 19 verifizierte DE-Beziehungen. Diese Erweiterung erlaubt künftig echte Vergleichslogik für fünf Kernspiele, aktiviert aber weder Affiliate-Links noch rechtlich ungeprüfte Offers.

## D-024 – Echte Matchlogik vor UI-Aktivierung isolieren
Die reale Matchlogik wird als eigenes Modul vor der sichtbaren Ergebnisansicht gebaut. `match-results.mjs` akzeptiert im aktuellen MVP ausschließlich `DE + virtual_slots`, verlangt pro Spiel mindestens drei aktuelle, separat verifizierte Betreiber-Spielseiten und einen marktpassenden Anbieter mit aktuellem Lizenz-Prüfdatum. Der Score normalisiert nur tatsächlich gewählte Nutzerkriterien plus Datenfrische; Affiliate-Vergütung fließt nicht ein. Jeder erzeugte Datensatz trägt `affiliate.enabled = false`, bis eine separate Vertrags- und Rechtsfreigabe vorliegt. Spiele unter dem 3-Anbieter-Gate liefern keinen künstlichen Vergleich, sondern `insufficient_verified_coverage`. Erst nach diesem technischen Gate wird die sichtbare Demo-Ergebnisansicht ersetzt.
