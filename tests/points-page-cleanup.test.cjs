...
assert(index.includes('v24-points-balance.js?v=2403'),'index must load the refreshed stable balance panel');
...
assert(balances.includes('Diese Stände nutzt VAYQUO für deine Auswertungen.'),'balance panel must explain its purpose once');
assert(balances.includes('id="v24pb-programs">Programme verwalten</button>'),'program management must live beside balance management');
assert(balances.includes("txt(el)==='Programme ändern'"),'new program-management action must delegate to the existing working control');
