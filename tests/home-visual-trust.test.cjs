...
assert(homeUsp.includes('v44-home-visual-trust.js?v=4409'),'existing home USP module must load the isolated visual module');
...
assert(moduleSource.includes("card:'assets/vayquo-card-wallet.webp?v=1'"),'card tile must use the approved real wallet asset');
assert(!moduleSource.includes('data:image/webp;base64'),'card tile must not carry a huge embedded data URL');
...
console.log('VAYQUO home visual gates: OK (real wallet asset; no embedded data URL; legacy entry paint-gated; card-check flow preserved)');
