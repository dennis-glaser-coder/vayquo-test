...
function showResult(){setScreen('result');$('#vqResultTitle').textContent=`Deine erste Einschätzung für ${PROJECTS[state.project]} ist bereit.`.replace('[object Object]',PROJECTS[state.project].label);$('#vqProfileSummary').innerHTML=summaryRows().map(row=>`<div class="vq-profile-row"><span>${row.key}</span><b>${row.value}</b></div>`).join('');progress();emit('revenue_result')}
