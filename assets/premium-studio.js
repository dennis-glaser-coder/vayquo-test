...
function format(n){return new Intl.NumberFormat('de-DE',{maximumFractionDigits:1}).format(n)}
...
function render(){if(!canvas)return;const {w,d}=dim();if(!current.cat||!current.chosen.length){canvas.innerHTML='<div class="empty-room"><span>Wähle ein Setup</span><small>Danach zeigt VAYQUO hier die geplante Stellfläche.</small></div>';updateLabels(0);return}
  canvas.innerHTML='';
  const items=current.chosen.map((x,i)=>{const fp=FOOTPRINTS[current.cat]?.[x.id]||[.55,.45];return {...x,fw:fp[0],fd:fp[1],i}}).sort((a,b)=>(b.fw*b.fd)-(a.fw*a.fd));
  ...
  for(const item of items){used+=item.fw*item.fd;const iw=Math.min(84,(item.fw/w)*100),ih=Math.min(70,(item.fd/d)*100);if(x+item.fw>w-.18){x=.18;y+=rowH+.18;rowH=0}if(y+item.fd>d-.18){x=Math.max(.18,(item.i%4)*(.55));y=Math.max(.18,d-.18-item.fd)}
    const el=document.createElement('div');el.className='room-item';el.style.left=`${(x/w)*100}%`;el.style.top=`${(y/d)*100}%`;el.style.width=`${Math.max(8,iw)}%`;el.style.height=`${Math.max(7,ih)}%`;el.innerHTML=`<b>${shortName(item.name)}</b>`;el.title=`${item.name} · Planfläche ca. ${format(item.fw)} × ${format(item.fd)} m`;canvas.appendChild(el);x+=item.fw+.18;rowH=Math.max(rowH,item.fd)}
  updateLabels(used)
}
