...
form.addEventListener('submit',async e=>{
 ...
 const payload={company_name:clean(fd.get('company_name'),120),email:clean(fd.get('email'),200),website:clean(fd.get('website'),250)||null,categories,regions:clean(fd.get('regions'),250)||null,project_min_eur:minRaw?Number(minRaw):null,note:clean(fd.get('note'),1000)||null,source:'partner_page'};
 ...
 try{
   ...
   form.reset();status.className='p-status ok';status.textContent='Danke. Ihre Vormerkung wurde gespeichert. VAYQUO meldet sich, sobald der passende Partneraufbau startet.';btn.textContent='Vorgemerkt ✓';
   window.dispatchEvent(new CustomEvent('vayquo:partner_interest',{detail:{categories:categories.join('.')}}));
   setTimeout(()=>{btn.disabled=false;btn.textContent='Vormerken →'},2500);
 }catch{
   status.className='p-status err';status.textContent='Das hat gerade nicht funktioniert. Bitte später erneut versuchen oder webmaster@vayquo.de kontaktieren.';btn.disabled=false;btn.textContent='Vormerken →';
 }
});
})();