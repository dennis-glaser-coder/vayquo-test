...
function isCoreTarget(v){return ['mr','pb','mm'].includes(v);}
...
function ensureStyle(){
 ...
 style.textContent=`
  ...
  .v25cb-target-existing{display:none!important}
  .v25cb-target-existing.is-visible{display:block!important}
...
function priorityTargets(ids){
 const order=['flying_blue','ba_club','qatar_privilege','emirates_skywards','singapore_krisflyer','sas_eurobonus','cathay','delta_skymiles','iberia_club','finnair_plus','aerclub','vueling_club','loganair_loyalty'];
 return [...ids].sort((a,b)=>{const ai=order.indexOf(a),bi=order.indexOf(b);if(ai!==-1||bi!==-1)return (ai===-1?999:ai)-(bi===-1?999:bi);return programName(a).localeCompare(programName(b),'de');});
}
function enhanceSelect(){
 const sel=q('#v24os-currency');if(!sel||!rules||sel.dataset.v25Bridge==='1')return;
 sel.dataset.v25Bridge='1';

 // Kein unscharfes "anderes Programm": VAYQUO soll nur rechnen, wenn das Programm bekannt ist.
 q('option[value="other"]',sel)?.remove();
 if(!q('option[value=""]',sel)){
  const placeholder=document.createElement('option');
  placeholder.value='';placeholder.textContent='Programm auswählen';placeholder.disabled=true;placeholder.selected=true;
  sel.insertBefore(placeholder,sel.firstChild);
 }

 if(mrActive()){
  const targets=priorityTargets(window.VayquoEngine.reachableMrTargets(rules));
  if(targets.length){
   const group=document.createElement('optgroup');group.label='Airline-Partner via Membership Rewards';
   targets.forEach(id=>{const opt=document.createElement('option');opt.value=id;opt.textContent=programName(id);group.appendChild(opt);});
 ...
 if(form&&!q('#v25cb-existing',form)){
  ...
  label.innerHTML='<span id="v25cb-existing-label">Bereits im Zielprogramm vorhanden</span><div class="v24os-control"><input id="v25cb-existing" type="number" min="0" step="1" inputmode="numeric" placeholder="0"></div>';
  const award=q('#v24os-award')?.closest('label');
  award?.insertAdjacentElement('afterend',label);
 }
 const sync=()=>{
  const value=String(sel.value||'');
  const concrete=!!value&&!isCoreTarget(value);
  q('#v25cb-existing-wrap')?.classList.toggle('is-visible',concrete);
  const label=q('#v25cb-existing-label');
  if(label)label.textContent=concrete?`Bereits bei ${programName(value)} vorhanden`:'Bereits im Zielprogramm vorhanden';
  if(!concrete&&q('#v25cb-existing'))q('#v25cb-existing').value='';
 };
...
async function calculate(ev){
 ...
 enhanceSelect();
 const currency=String(q('#v24os-currency')?.value||'');
 const out=q('#v24os-result');
 if(!currency){if(out)out.innerHTML='<div class="v24os-error">Bitte zuerst das Punkte- oder Meilenprogramm auswählen.</div>';return;}
